    ...
    <div class="right-dy-data">
        ...
        <form role="form" class="form-inline" enctype="multipart/form-data" id="data_form">
            <div class="right-inblock-div">
                ...
                <div class="cell"><label>标题：</label><input type="text" validrule="validate[required,maxSize[20]]" name="mtitle"></div>
                <c:if test="${bean.navigationid2!=1 && bean.navigationid2!=14}">
                    <div class="cell"><label>是否上大banner条：</label><input type="radio" name="mbanner" value="0" checked/>否 <input type="radio" name="mbanner" value="1"/>是 </div>
                </c:if>
                <c:if test="${bean.navigationid2!=1 && bean.navigationid2!=2 && bean.navigationid2!=3 && bean.navigationid2!=4 && bean.navigationid2!=14}">
                    <div class="cell"><label>是否上新闻中心：</label><input type="radio" name="mmsgc" value="0" checked/>否 <input type="radio" name="mmsgc" value="1"/>是 </div>
                </c:if>
                <div class="cell"><label>关键字：</label><input type="text" validrule="validate[required,maxSize[120]]" name="mtags" placeholder="逗号分隔"></div>
                <c:if test="${bean.navigationid2!=1 && bean.navigationid2!=14}">
                    <div class="cell"><label>展示图：</label><input type="file" name="fileimage" style="display: inline-block;" accept="image/*" validrule="validate[required]"></div>
                </c:if>
                <div style="float: left;width: 90%;height: 40px;padding:5px 0 0 0;"><label>摘要信息：</label><input style="width: 600px;" type="text" validrule="validate[required,maxSize[100]]" name="elliptical" placeholder="摘要信息最多100字"></div>
                <textarea name="context" style="width: 90%;height: 350px;"></textarea>
            </div>
