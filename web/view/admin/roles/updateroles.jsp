    ...
    <div class="right-dy-data">
        ...
        <form class="form-inline" role="form" id="national_form">
            <div class="right-inblock-div">
                ...
                <div class="cell"><label>角色名称：</label><input type="text" validrule="validate[required,maxSize[20]]" name="rolename" value="${Role.rolename}"></div>
            </div>
                ...
                <div class="account-div">
                    <c:forEach items="${MenuList}" var="first">
                        <div class="block-account">
                            <div class="first-level"><input type="checkbox" value="${first.nodeid}" <c:if test="${first.checked}">checked</c:if> onclick="checkAllBox('checknetall${first.nodeid}','checknet${first.nodeid}')" id="checknetall${first.nodeid}">${first.nodetext}</div>
                            <c:forEach items="${first.sonMenus}" var="son">
                                <div class="second-level"><input type="checkbox" value="${son.nodeid}" <c:if test="${son.checked}">checked</c:if>  name="checknet${first.nodeid}">${son.nodetext}</div>
                            </c:forEach>
                        </div>
                    </c:forEach>
                </div>
            ...
            <div class="right-btn">
                <button class="btn btn-default" onclick="validFormData(${Role.roleid})" type="button">保存</button>
                <button class="btn btn-default" onclick="gotoRolesDetailPage(${Role.roleid},1)" style="margin-left: 20px;" type="button">取消</button>
            </div>
        </form>
    </div>
</div>
