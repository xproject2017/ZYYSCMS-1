...
function callBackPagination(currPage, _falg) {
    $.ajax({
        ...
        url: '/view/admin/pbespke/getBespkeListData',
        ...
        success: function (datalist) {
            ...
            if (datalist.code == 0) {
                ...
                $.each(datalist.result, function (i, item) {
                    ...
                    html.push('<div class="plan-cell"><span style="cursor: pointer;text-decoration: underline;" onclick="getUpdateDataPage('+item.bespokeid+')">'+item.pbespokename+'</span></div>');
                    html.push('<div class="plan-cell">' + item.pbespokephone + '</div>');
                    html.push('<div class="plan-cell">' + item.pbespoketimestr +'</div>');
                    html.push('<div class="plan-cell">' + item.productname +'</div>');
...
function getUpdateDataPage(bespokeid){
    window.location.href="/view/admin/pbespke/updateBespkePage?bespokeid="+bespokeid+"&nodetext="+$("h4").html();
}
...
function updateFormData(){
    ...
    if(_form.validationEngine('validate')){
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/pbespke/updateData',
            ...
            success: function(data) {
                if(data.code == 0) {
                    jAlert("修改成功", "提示",function(r){
                        if(r){
                            window.location.href = "/view/admin/pbespke/getBespkeListPage?nodetext="+$("h4").html();
                        }
                    });
                }else{
