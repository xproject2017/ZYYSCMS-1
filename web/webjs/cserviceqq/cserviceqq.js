...
function callBackPagination(currPage, _falg) {
    ...
    $.ajax({
        ...
        url: '/view/admin/qqservice/getCserrviceqqListData',
        ...
        success: function (datalist) {
            ...
            if (datalist.code == 0) {
                ...
                $.each(datalist.result, function (i, item) {
                    ...
                    html.push('<div class="plan-cell"><span style="cursor: pointer;text-decoration: underline;" onclick="getUpdateDataPage('+item.id+')">'+item.qq+'</span></div>');
                    html.push('<div class="plan-cell">' + item.cname +'</div>');
                    html.push('<div class="plan-cell"><a href="javascript:delData('+item.id+')">删除</a> </div>');
                    html.push('</div>');
                });
            } else {
...
function delData(_mesgid){
    $.post("/view/admin/qqservice/delData",{"id":_mesgid},function(data){
        if(data.code == 0) {
...
function getUpdateDataPage(mesgid){
    window.location.href="/view/admin/qqservice/updateCserrviceqqPage?id="+mesgid+"&nodetext="+$("h4").html();
}

function getAddDataPage(){
    window.location.href="/view/admin/qqservice/addCserrviceqqListPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
}
...
function updateFormData(){
    ...
    if(_form.validationEngine('validate')){
        ...
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/qqservice/updateData',
            ...
            success: function(data) {
                if(data.code == 0) {
                    jAlert("修改成功", "提示",function(r){
                        if(r){
                            window.location.href = "/view/admin/qqservice/getCserrviceqqListPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
                        }
                    });
                }else{
                    jAlert(data.failinfo,"提示");
