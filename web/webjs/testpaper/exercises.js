...
function callBackPagination(currPage, _falg) {
    ...
    $.ajax({
        ...
        url: '/view/admin/testpaper/getExercisesListData',
        ...
        success: function (datalist) {
            ...
            if (datalist.code == 0) {
                ...
                $.each(datalist.result, function (i, item) {
                    ...
                    html.push('<div class="plan-cell">'+item.etitle+'</div>');
                    var _str =  item.etype==1?'公司':'个人';
                    html.push('<div class="plan-cell">' +_str + '</div>');
                    html.push('<div class="plan-cell"><a href="javascript:delData('+item.eid+','+tid+')">移除</a> </div>');
                    html.push('</div>');
                });
            } else {
...
function delData(_mesgid,tid){
    $.post("/view/admin/testpaper/delFrompaper",{"eid":_mesgid,"tid":tid},function(data){
        if(data.code == 0) {
            jAlert("移除成功","",function(r){
                if(r){
...
function getUpdateDataPage(mesgid){
    window.location.href="/view/admin/exercises/updateExercisesPage?eid="+mesgid+"&nodetext="+$("h4").html();
}

function getAddDataPage(){
    window.location.href="/view/admin/testpaper/addExercisesPage?tid="+$("[name='tid']").val()+"&nodetext="+$("h4").html();
}
