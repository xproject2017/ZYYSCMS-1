...
function callBackPagination(currPage, _falg) {
    ...
    $.ajax({
        ...
        url: '/view/admin/industry/getIndustrynewsListData',
...
function getUpdateDataPage(mesgid){
    window.location.href="/view/admin/industry/updateIndustrynewsPage?mesgid="+mesgid+"&nodetext="+$("h4").html();
}

function getAddDataPage(){
    window.location.href="/view/admin/industry/addIndustrynewsPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
}
            ...
            success: function(data) {
                if(data.code == 0) {
                    jAlert("修改成功", "提示",function(r){
                        if(r){
                            window.location.href = "/view/admin/industry/getIndustrynewsListPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
                        }
                    });
                }else{
                    jAlert(data.failinfo,"提示");
...
function addFormData(){
    ...
    if(_form.validationEngine('validate')){
        ...
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/industry/addnewsData',
            data:formData,// 你的formid
            processData: false,
            contentType: false,
            error: function(XMLHttpRequest, textStatus, errorThrown) {
