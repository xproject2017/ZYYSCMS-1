...
function callBackPagination(currPage, _falg) {
    ...
    $.ajax({
        ...
        url: '/view/admin/industry/getIndustrytendencyListData',
...
function delData(_mesgid){
    $.post("/view/admin/industry/delData",{"mesgid":_mesgid},function(data){
        if(data.code == 0) {
...
function getUpdateDataPage(mesgid){
    window.location.href="/view/admin/industry/updateIndustrytendencyPage?mesgid="+mesgid+"&nodetext="+$("h4").html();
}

function getAddDataPage(){
    window.location.href="/view/admin/industry/addIndustrytendencyPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
}
...
function updateFormData(){
    ...
    if(_form.validationEngine('validate')){
        ...
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/industry/updateData',
            ...
            success: function(data) {
                if(data.code == 0) {
                    jAlert("修改成功", "提示",function(r){
                        if(r){
                            window.location.href = "/view/admin/industry/getIndustrytendencyListPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
                        }
                    });
                }else{
                    jAlert(data.failinfo,"提示");
...
function addFormData(){
    ...
    if(_form.validationEngine('validate')){
        ...
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/industry/addtendencyData',
            data:formData,// 你的formid
            processData: false,
            contentType: false,
            error: function(XMLHttpRequest, textStatus, errorThrown) {
