...
function callBackPagination(currPage, _falg) {
    $.ajax({
        ...
        url: '/view/admin/pnotice/getNoticeListData',
...
function delData(_mesgid){
    $.post("/view/admin/pnotice/delData",{"mesgid":_mesgid},function(data){
        if(data.code == 0) {
...
function getUpdateDataPage(mesgid){
    window.location.href="/view/admin/pnotice/updateNoticePage?mesgid="+mesgid+"&nodetext="+$("h4").html();
}

function getAddDataPage(){
    window.location.href="/view/admin/pnotice/addNoticePage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
}
...
function updateFormData(){
    ...
    if(_form.validationEngine('validate')){
        ...
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/pnotice/updateData',
            ...
            success: function(data) {
                if(data.code == 0) {
                    jAlert("修改成功", "提示",function(r){
                        if(r){
                            window.location.href = "/view/admin/pnotice/getNoticeListPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
                        }
                    });
                }else{
                    jAlert(data.failinfo,"提示");
...
function addFormData(){
    ...
    if(_form.validationEngine('validate')){
        ...
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/pnotice/addData',
            data:formData,// 你的formid
            processData: false,
            contentType: false,
            error: function(XMLHttpRequest, textStatus, errorThrown) {
            ...
            success: function(data) {
                if(data.code == 0) {
                    jAlert("提交成功", "提示",function(r){
                        if(r){
                            window.location.href = "/view/admin/pnotice/getNoticeListPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val();
                        }
                    });
                }else{
                    jAlert(data.failinfo,"提示");
