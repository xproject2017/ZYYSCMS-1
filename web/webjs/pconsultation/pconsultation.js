...
function callBackPagination(currPage, _falg) {
    $.ajax({
        ...
        url: '/view/admin/pconsultation/getConsultationListData',
        ...
        success: function (datalist) {
            ...
            if (datalist.code == 0) {
                ...
                $.each(datalist.result, function (i, item) {
                    ...
                    html.push('<div class="plan-cell"><span style="cursor: pointer;text-decoration: underline;" onclick="getUpdateDataPage('+item.consid+')">'+item.consname+'</span></div>');
                    html.push('<div class="plan-cell">' + item.consphone +'</div>');
                    html.push('<div class="plan-cell">' + item.constimestr + '</div>');
                    html.push('<div class="plan-cell">' + item.mes +'</div>');
...
function getUpdateDataPage(consid){
    window.location.href="/view/admin/pconsultation/updateConsultationPage?consid="+consid+"&nodetext="+$("h4").html();
}
...
function updateFormData(){
    ...
    if(_form.validationEngine('validate')){
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/pconsultation/updateData',
            ...
            success: function(data) {
                if(data.code == 0) {
                    jAlert("修改成功", "提示",function(r){
                        if(r){
                            window.location.href = "/view/admin/pconsultation/getConsultationListPage?nodetext="+$("h4").html();
                        }
                    });
                }else{
