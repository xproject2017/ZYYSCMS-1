...
function callBackPagination(currPage, _falg) {
    $.ajax({
        ...
        url: '/view/admin/recruit/getRecruitListData',
...
function delData(_mesgid){
    $.post("/view/admin/recruit/delData",{"mesgid":_mesgid},function(data){
        if(data.code == 0) {
...
function getUpdateDataPage(mesgid){
    window.location.href="/view/admin/recruit/updateRecruitPage?mesgid="+mesgid+"&nodetext="+$("h4").html();
}

function getAddDataPage(){
    window.location.href="/view/admin/recruit/addRecruitPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
}
...
function updateFormData(){
    ...
    if(_form.validationEngine('validate')){
        ...
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/recruit/updateData',
            ...
            success: function(data) {
                if(data.code == 0) {
                    jAlert("修改成功", "提示",function(r){
                        if(r){
                            window.location.href = "/view/admin/recruit/getRecruitListPage?navigationid1="+$("[name='navigationid1']").val()+"&navigationid2="+$("[name='navigationid2']").val()+"&nodetext="+$("h4").html();
                        }
                    });
                }else{
                    jAlert(data.failinfo,"提示");
...
function addFormData(){
    ...
    if(_form.validationEngine('validate')){
        ...
        $.ajax({
            type: "POST",
            dataType:"json",
            url:'/view/admin/recruit/addData',
            data:formData,// 你的formid
            processData: false,
            contentType: false,
            error: function(XMLHttpRequest, textStatus, errorThrown) {
